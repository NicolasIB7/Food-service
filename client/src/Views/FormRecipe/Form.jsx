...
const Form = () => {
  ...
  const [form, setForm] = useState({
    // Creo un estado con los datos de mis inputs, el formulario debe ser un reflejo del estado, el formulario tiene
  ...
  return (
    <div className={style.div}>
      <form onSubmit={submitHandler} className={style.divForm}>
        ...
        <div>
          <select onChange={handleSelect} className={style.diet}>
            <option value={form.diets} name='diets'>
              Diet type
            </option>
            {diet?.map((c) => {
        ...
        errors.steps ||
        errors.healthScore ? (
          <button disabled='disabled' className={style.disabled}>
            CREATE
          </button>
        ) : (
      ...
      <div className={style.dietas}>
        {form.diets.map((el) => (
          <div>
            <button onClick={() => handleDelete(el)} className={style.x}>
              X
            </button>
